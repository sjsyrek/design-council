...
- **Review-mode logs look different.** If this council ran in Review mode (see `SKILL.md` "Review mode variant"), the "Opening verdicts" and "Resolved disagreements" sections are typically empty — reviews produce findings, not verdicts, and seats rarely cross-talk. Replace those sections with "Per-seat finding counts" (P0/P1 breakdown) and "CEO dedup decisions" (overlapping findings merged into fewer tracker items, with brief rationale). The Execution plan becomes the primary artifact: a file-ownership map of the filed tracker items so downstream workers don't collide when addressing findings. Also add an "Un-spawned seats / uncovered domains" section if Phase 2.5 handshake verification dropped any seats — future maintainers need to know which lanes were not reviewed.
